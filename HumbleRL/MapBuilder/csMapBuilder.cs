...
namespace HumbleRL.MapBuilder
{
	...
	class csMapbuilder
	{
		...
		private int doorcell = 3;
		private int corridorcell = 2;
		private int wallcell = 1;
		private int floorcell = 0;
		private int empty = -1;
		...
		public csMapbuilder( int x, int y )
		{
			...
			Corridor_Max = 15;
			MaxRooms = 15;
			Map_Size = new Size( Map_Size.Width, Map_Size.Height );
		...
		public bool Build_OneStartRoom()
		{
			...
			SurroundRoomsWithWalls();

			return true;
		}

		private void SurroundRoomsWithWalls() {
			System.Console.WriteLine( "Surroung" );
		
			foreach ( Rectangle room in rctBuiltRooms )
			{
				room.Inflate( 2, 2 );
				//room.Offset( -1, -1 );
					...
					for ( int y = room.Top; y <= room.Bottom; y++ )
					{
						if ( x == room.Left || x == room.Right || y == room.Top || y == room.Bottom )
						{
							if ( map[x, y] == empty )
							{
								...
								map[x, y] = doorcell;
							//System.Console.WriteLine( "Nowall" );
						}
		...
		private void Room_GetEdge( out Point pLocation, out Point pDirection )
		{
			...
			} while ( Point_Get( pLocation.X, pLocation.Y ) != empty );
			...
			do
			{
				...
				foreach ( Point p in directions_straight )
					if ( Point_Valid( pLocation.X + p.X, pLocation.Y + p.Y ) )
						if ( Point_Get( pLocation.X + p.X, pLocation.Y + p.Y ) == empty )
		...
		private void Corridor_Build()
		{
			...
			{
				Point_Set( p.X, p.Y, corridorcell );
		...
		private bool Room_Verify()
		{
			...
			for ( int x = rctCurrentRoom.Left; x <= rctCurrentRoom.Right; x++ )
				for ( int y = rctCurrentRoom.Top; y <= rctCurrentRoom.Bottom; y++ )
					if ( !Point_Valid( x, y ) || Point_Get( x, y ) != empty )
						return false;
		...
		private void Room_Build()
		{
			...
			for ( int x = rctCurrentRoom.Left - 1; x <= rctCurrentRoom.Right + 1; x++ )
				for ( int y = rctCurrentRoom.Top - 1; y <= rctCurrentRoom.Bottom + 1; y++ )
				{
					//if ( x == rctCurrentRoom.Left - 1 || x == rctCurrentRoom.Right + 1 || y == rctCurrentRoom.Top - 1 || y == rctCurrentRoom.Bottom + 1 )
					//	map[x, y] = wallcell;
					//else
					map[x, y] = floorcell;
				}
